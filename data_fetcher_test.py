...
class TestGetUserProfile(unittest.TestCase):
    ...
    def test_get_user_profile_success(self, mock_bigquery_client):
        """Test successfully retrieving a user profile with friends."""

        # Create a mock row that behaves like a BigQuery row (dict-style)
        mock_user_row = {
            'UserId': 'user1',
            'Name': 'Remi',
        ...
        mock_client_instance.query.side_effect = [
            MagicMock(result=MagicMock(return_value=iter([mock_user_row]))),
            MagicMock(result=MagicMock(return_value=iter(mock_friend_rows)))
        ]

        # Make the mock client return this mocked instance
