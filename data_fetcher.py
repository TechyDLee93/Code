...
def get_user_profile(user_id):
    ...
    Output: A single dictionary with keys full_name, username, date_of_birth, profile_image, and friends (list of friend user_ids)
    """
    client = bigquery.Client()
    ...
    user_query = """
        ...
        FROM `keishlyanysanabriatechx25.bytemeproject.Users`
    ...
    friends_query = """
        ...
        FROM `keishlyanysanabriatechx25.bytemeproject.Friends`
    ...
    # AI Citation: Help from ChatGPT was used in structuring the UNION logic for mutual friendship retrieval.
    user_result = client.query(user_query, job_config=job_config).result()
    user_row = next(iter(user_result), None)
    if user_row is None:
        raise ValueError(f"User {user_id} not found.")

    friends_result = client.query(friends_query, job_config=job_config).result()
    friends = [row['FriendId'] for row in friends_result]

    return {
        'full_name': user_row['Name'],
        'username': user_row['Username'],
        'date_of_birth': user_row['DateOfBirth'].strftime('%Y-%m-%d') if user_row['DateOfBirth'] else None,
