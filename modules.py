...
def goal_progress_tracking_ui(user_id, task_id, start_date, end_date):
    ...
    new_data = read_task_completion_from_gcs(user_id, task_id)
    ...
    if selected_date:
        ...
        if new_data:
            tasks_for_day = new_data.get(day_label, [])
        else:
    ...
    if new_data:
        for day_tasks in new_data.values():
            for task in day_tasks:
                total_tasks += 1
                if task.get("completed", False):
                    completed_tasks += 1
    else:
        for day_tasks in progress_data.values():
            for task in day_tasks:
                total_tasks += 1
                if task.get("completed", False):
                    completed_tasks += 1
    ...
    if total_tasks > 0:
        ...
        st.markdown(f"**Overall Progress:** {int(progress_percent * 100)}% completed")
        st.progress(progress_percent)
    else:
        st.info("No tasks available to track progress.")
